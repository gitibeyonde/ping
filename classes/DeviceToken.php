...
class DeviceToken
{
    ...
    public function addDeviceToken($user_name, $phone_id, $token, $system, $system_type, $language, $country, $endpoint_arn){
        ...
        if ($this->databaseConnection()) {
            // database query, getting all the info of the selected user
            $sth = $this->db_connection->prepare('insert into device_token(user_name, phone_id, token, endpoint_arn, system, system_type, language, country, updated)  values(:user_name, :phone_id, :token, :endpoint_arn, :system, :system_type, :language, :country, now())');
