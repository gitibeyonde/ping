...
class Login
{
    ...
    private $user_name = "";
    /**
     * @var string $role The user's role
     */
    private $role = "";  // ADMIN,USER,PAID,DEV
    /**
    ...
    private function loginWithSessionData()
    {
        ...
        if (isset($_SESSION)){
            ...
            $this->user_otp = $_SESSION['user_otp'];
        }
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email, role, user_phone, user_otp FROM users WHERE user_id = :user_id
                    ...
                    if (isset($result_row->user_id)) {
                        ...
                        $_SESSION['user_otp'] = $result_row->user_otp;
                        ...
                        $this->user_otp = $result_row->user_otp;
                        $this->user_phone = $result_row->user_phone;
                        $this->user_is_logged_in = true;
                        $_SESSION['view']= MAIN_VIEW;
    ...
    {
        if (!$this->user_is_logged_in)return;
     ...
     * Create all data needed for remember me cookie connection on client and server side
     */
    public function saveOtp($user_otp)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            $sth = $this->db_connection->prepare("UPDATE users SET user_otp = :user_otp WHERE user_id = :user_id");
            $sth->execute(array(':user_otp' => $user_otp, ':user_id' => $_SESSION['user_id']));
            error_log("saveOtp Error=" . implode(",", $sth->errorInfo()));
     ...
     * Create all data needed for remember me cookie connection on client and server side
     */
    public function savePhoneNumber($user_phone)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            $sth = $this->db_connection->prepare("UPDATE users SET user_phone = :user_phone WHERE user_id = :user_id");
            $sth->execute(array(':user_phone' => $user_phone, ':user_id' => $_SESSION['user_id']));
            error_log("saveOtp Error=" . implode(",", $sth->errorInfo()));
    ...
    public function sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash)
    {
        ...
        error_log($user_email." Msg id=".$msg_id);
        return true;
